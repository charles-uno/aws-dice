...
func (clone gameState) playHelper(c card) []gameState {
    ...
    switch c.name {
        ...
        case "Temple of Mystery":
            return clone.playTempleOfMystery()
        case "Urza's Saga":
...
func (self *gameState) playTempleOfMystery() []gameState {
    ret := []gameState{}
    milledRaw, library := self.library.SplitAfter(1)
    milled := CardMap(milledRaw)
    // Keep the top card on top
    clone := self.clone()
    clone.logText(", keep ")
