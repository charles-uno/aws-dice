...
func NewGame(libraryRaw []string, handRaw []string, otp bool, verbose bool, maxTurns int) (gameManager, error) {
    // Before firing up an actual game state, collect all the card names and
    // make sure we have data for them
    allCardNames := []string{}
    handCards := []card{}
    for _, cardName := range handRaw {
        // Swap out wildcards for actual draws
        if cardName == "WILDCARD" {
            cardName = libraryRaw[0]
            libraryRaw = libraryRaw[1:]
        }
